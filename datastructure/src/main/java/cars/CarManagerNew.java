 ...
 */
public class CarManagerNew extends AbstractManager<Car> {
    /** 单例模式的实例（复用父类 _instance，此处仅为兼容原有命名） */
    private static CarManagerNew _instance;

    /** 私有构造函数，注册 JVM 关闭钩子以确保在程序退出时保存车辆数据 */
    private CarManagerNew() {
        super();
     ...
     */
    public static CarManagerNew getInstance() {
        if (_instance == null) {
           _instance = new CarManagerNew();
        }
        return _instance;
    ...
    public int getRequiredPermissionLevel() {
        // 根据实际业务需求返回权限等级，例如：管理员等级为 2，普通用户为 1
