...
public class CarManager extends AbstractManager<Car> {
     ...
     * 如果有订单关联该车辆，则不允许删除（通过 OrderManager 查询订单关联关系），否则调用父类 remove 方法删除车辆数据
     */
    @Override
    public boolean remove(Car car) {
        ...
        var orders = OrderManager.getInstance().getOrderByCarID(car.getID());
        if (orders != null && !orders.isEmpty()) {
            return false; // 如果有订单关联该车辆，则不允许删除
        }
        if (!isEnabled(getRequiredPermissionLevel(), getAccountLevel())) {
            return false; // 权限校验
        }
        boolean result = items.remove(car.getID()) != null;
        return result;
    }
