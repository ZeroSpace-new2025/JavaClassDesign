...
public class OrderManager extends AbstractManager<Order> {
    ...
    public boolean remove(Order order) {
        ...
        CarManager.getInstance().getByID(order.getCarID()).setState(cars.CarState.AVAILABLE);
        if (result) {
            save();
        }
        return result;
    ...
    public void updateOrderByPrice(long ID, double newPrice) {
        ...
        if (order != null) {
            ...
            update(order); // 复用父类的 update 方法更新数据
        }
    }
    ...
    public boolean RentByBooking(long ID) {
        Order order = getByID(ID);
        if (order != null && order.getState() == OrderState.REVIEWING && order.getNature() == OrderNature.BOOKING) {
